...
# solcash Multi-installer
# a one line clone-and-compile for solcash:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/solcashorg/solcash/master/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/solcash ]; then
            read -r -p "${1:-solcash directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old solcash directory" && echo
                    rm -rf "$PWD"/solcash
                    ;;
                *)
                    _fail "solcash directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir solcash
        git clone -q https://github.com/solcashorg/solcash solcash   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd solcash
    fi
}

_build_solcash() {
    _note "Building solcash from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/solcashorg')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/solcashorg')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "solcash Multi_Installer v1.0 (pepperoni)"
_colorize green "SOLCASH" && echo
...
_note "Look in 'solcash/build/src/' for the executible binaries. See 'https://github.com/solcashorg/solcash' for more project support. !"
